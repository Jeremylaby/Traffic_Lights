...
const TrafficLight = ({ state, rotation = 0, broken = false }: TrafficLightProps)=> {
    ...
    return (
        <g transform={`rotate(${rotation})`}>
            ...
            <Bulb cx={cx} cy={yellowCy} r={r} $color={broken ? '#ffd700' : config.yellow} $active={config.yellow == DIM || broken} $blink={broken} />
            <Bulb cx={cx} cy={greenCy} r={r} $color={broken ? DIM : config.green}  $active={config.green == DIM && !broken} />
